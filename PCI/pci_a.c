...
#define PCI_ADDRESS_OFFSET (DWORD) (PCI_ADDRESS | (offset << 2))
...
int PCI_read_in_byte_keyboard (int k);
...
void PCI_read_in_byte (int bus, int device, int function) {
	DWORD data;
	WORD data_0, data_1, data_2, data_3;
	int offset = 0;
	int i = 0;
  textcolor (GREEN);
	system ("cls");
...
//PCI_read_in_byte_keyboard
int PCI_read_in_byte_keyboard (int k) {    //k receive
	int x = 4, y = 5;
...
/* Modify PCI */
void PCI_modify (int length, int offset, int k) {
  DWORD modify_input [10];
  DWORD input_change = 0;
  DWORD data_read;
  DWORD data_change;
  int i = 1;
  int bus = a [k];
